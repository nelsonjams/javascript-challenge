...
tableData.forEach((sightings) => {
    // Creating table rows for each row of sightings data
...
filterType.on("change", function() {
    ...
    d3.select("label")
      ...
      .text(`ENTER A VALUE FOR  ${filterValue.toLowerCase()}`);//IS THERE A WAY TO DO EXCEPT FOR DATETIME WHICH SHOULD BE REPLACED WITH DATE??
...
submit.on("click", function() {
        ...
        var filteredData = tableData.filter(sightings => sightings[typeVal] === inputValue.toLowerCase());
        if (filteredData.length == 0) {
            alert("NO UFO's FOUND, TRY A DIFFERENT SEARCH TERM!");
            d3.select("#filtertype").node().value = '';
            autoPopulate(tableData);
        }
